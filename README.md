...
7. **Если начинаешь сессию не с чистого листа** — загляни в [docs/zhurnal-izmeneniy.md](docs/zhurnal-izmeneniy.md), последняя запись: там может быть список незавершённых с прошлой сессии пунктов.
...
3. [Портянка и рабочий график](docs/reglament/03-portyanka-grafik.md) — разделы 5–6
4. [Отношения](docs/reglament/04-otnosheniya.md) — раздел 7
...
- Хаб: отдельного промпта нет, работает напрямую от LPF-регламента (включая часть 1 — временная привязка).
...
- **Распожаризация (с 30.08.2026):** локальные рабочие файлы по заданиям курса — в [raspozharizaciya/](raspozharizaciya/), начиная с `r1-1-tasks1-opisanie-sistemy.md`. Сквозной пример курса — венчурный фонд deeptech-инвестиций (реальный проект Марии, он же ЦПИ/стартап-студия), поэтому релевантные разборы также дублируются проектной выжимкой в `startup-studio/` — см. раздел «Проекты — рабочий контекст».
